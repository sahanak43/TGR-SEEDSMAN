 ...
 * Slider component
 * @class Slider
 * @namespace Seedsman/Component/Slider/Component */
export class SliderComponent extends PureComponent {
    ...
    renderCounter() {
        const {
            children, showCounter, activeImage, showArrows, isPdp
        } = this.props;

        if (!showCounter || children.length <= 1 || !showArrows) {
            return null;
        }
    ...
    renderArrows() {
        const {
            showArrows, activeImage, children, isPdp
        } = this.props;
        const nextIsDisabled = activeImage + 1 === children.length;
        const prevIsDisabled = activeImage === 0;
