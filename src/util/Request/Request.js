...
/** @namespace Seedsman/Util/Request/getWindowId */
...
/** @namespace Seedsman/Util/Request/getStoreCodePath */
export const getStoreCodePath = () => {
    const path = location.pathname;
    // eslint-disable-next-line no-undef
    const firstPathPart = path.split('/')[1];
    if (window.newStoreList.includes(firstPathPart)) {
        return `/${ firstPathPart }`;
    }

    return '';
};
/** @namespace Seedsman/Util/Request/getGraphqlEndpoint */
 ...
 * @namespace Seedsman/Util/Request/appendTokenToHeaders */
 ...
 * @returns {*}
 * @namespace Seedsman/Util/Request/formatURI */
 ...
 * @namespace Seedsman/Util/Request/getFetch */
 ...
 * @param {Int} cacheTTL
 * @namespace Seedsman/Util/Request/putPersistedQuery */
 ...
 * @param {String} name
 * @returns {Promise<Response>}
 * @namespace Seedsman/Util/Request/postFetch */
 ...
 * @namespace Seedsman/Util/Request/checkForErrors */
 ...
 * @namespace Seedsman/Util/Request/handleConnectionError */
 ...
 * @namespace Seedsman/Util/Request/parseResponse */
export const parseResponse = (promise) => new Promise((resolve, reject) => {
    promise.then(
        /** @namespace Seedsman/Util/Request/parseResponse/Promise/promise/then */
        (res) => res.json().then(
            /** @namespace Seedsman/Util/Request/parseResponse/Promise/promise/then/json/then/resolve */
            (res) => resolve(checkForErrors(res)),
            /** @namespace Seedsman/Util/Request/parseResponse/Promise/promise/then/json/then/catch */
            () => {
                handleConnectionError('Can not transform JSON!');
                return reject();
            }
        ),
        /** @namespace Seedsman/Util/Request/parseResponse/Promise/promise/then/catch */
        (err) => {
 ...
 * @return {Promise<Request>} Fetch promise to GraphQL endpoint
 * @namespace Seedsman/Util/Request/executeGet */
export const executeGet = (queryObject, name, cacheTTL) => {
    ...
    return parseResponse(new Promise((resolve, reject) => {
        getFetch(uri, name).then(
            /** @namespace Seedsman/Util/Request/executeGet/parseResponse/getFetch/then */
            (res) => {
                if (res.status === HTTP_410_GONE) {
                    putPersistedQuery(getGraphqlEndpoint(), query, cacheTTL).then(
                        /** @namespace Seedsman/Util/Request/executeGet/parseResponse/getFetch/then/putPersistedQuery/then */
                        (putResponse) => {
                            if (putResponse.status === HTTP_201_CREATED) {
                                getFetch(uri, name).then(
                                    /** @namespace Seedsman/Util/Request/executeGet/parseResponse/getFetch/then/putPersistedQuery/then/getFetch/then/resolve */
                                    (res) => resolve(res)
                                );
                            }
                        }
 ...
 * @param  {{}} queryObject prepared with `prepareDocument()` from `Util/Query` request body object
 * @return {Promise<Request>} Fetch promise to GraphQL endpoint
 * @namespace Seedsman/Util/Request/executePost */
...
/** @namespace Seedsman/Util/Request/executePostWithError */
export const executePostWithError = (queryObject) => {
    const { query, variables } = queryObject;
    ...
    return postFetch(getGraphqlEndpoint(), query, variables);
};
 ...
 * @namespace Seedsman/Util/Request/listenForBroadCast */
...
/** @namespace Seedsman/Util/Request/debounce */
...
/** @namespace Seedsman/Util/Request */
